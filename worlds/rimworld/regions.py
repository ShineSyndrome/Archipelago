...
class ExitNames:
    CAN_HITECH_RESEARCH = "CAN_HITECH_RESEARCH"
    CAN_MA_RESEARCH = "CAN_MA_RESEARCH"
...
def filter_locations_by_expansion(locations: Dict[int, RimWorldLocationData], options: RimWorldOptions) \
        -> Dict[int, RimWorldLocationData]:
    enabled_expansions = [Expansions.CORE]
    if options.biotech_expansion:
        enabled_expansions.append(Expansions.BIOTECH)
    if options.royalty_expansion:
        enabled_expansions.append(Expansions.ROYALTY)
    if options.ideology_expansion:
        enabled_expansions.append(Expansions.IDEOLOGY)
...
def create_region(world: MultiWorld, player: int, locations: Dict[int, RimWorldLocationData], name: str) -> Region:
    ...
    for loc_id, location_data in locations.items():
        location = RimWorldLocation(player, location_data.identifier(), loc_id, region)
